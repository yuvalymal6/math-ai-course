...
import { Brain, ChevronRight, Target, SlidersHorizontal } from "lucide-react";

const SUBTOPICS = [
  {
    id: "constraints",
    href: "/3u/topic/grade12/linear-programming/constraints",
    icon: SlidersHorizontal,
    title: "אילוצים ותחום אפשרי",
    subtitle: "אי-שוויונות • גרף • קודקודי התחום",
    description: "הגדרת אילוצים לינאריים, שרטוט התחום האפשרי במערכת צירים, ומציאת קודקודי הפוליגון.",
    chips: ["ax + by ≤ c", "תחום אפשרי", "קודקודים"],
    ready: false,
    glowAnim: false,
  ...
  {
    id: "objective",
    href: "/3u/topic/grade12/linear-programming/objective",
    icon: Target,
    title: "פונקציית מטרה",
    subtitle: "מקסימום • מינימום • בדיקת קודקודים",
    description: "הגדרת פונקציית המטרה z = ax + by, מציאת הערך המקסימלי והמינימלי על ידי בדיקת קודקודי התחום.",
    chips: ["z = ax + by", "מקסימום", "מינימום"],
    ready: false,
    glowAnim: false,
...
export default function LinearProgrammingHub() {
  return (
    <div className="min-h-screen bg-[#0a0f1e] text-white" dir="rtl">
      ...
      <header className="bg-[#0f172a] border-b border-slate-800 sticky top-0 z-10">
        <div className="max-w-2xl mx-auto px-4 py-3 flex items-center justify-between">
          ...
          <Link href="/" className="text-slate-400 hover:text-slate-200 text-sm transition-colors flex items-center gap-1">
            <ChevronRight size={14} className="rotate-180" />כיתה י״ב • 3 יח&quot;ל
          </Link>
        </div>
      </header>
      <main className="max-w-2xl mx-auto px-4 py-14 space-y-10">
        <div className="text-center space-y-3 animate-[fadeSlideIn_0.4s_ease_both]">
          <p className="text-blue-400 text-xs font-semibold uppercase tracking-widest">כיתה י״ב • 3 יח&quot;ל</p>
          <h1 className="text-4xl font-extrabold">תכנון לינארי</h1>
          <p className="text-slate-400 text-sm">אילוצים • פונקציית מטרה • מקסימום ומינימום</p>
        </div>
