...
function BasicSVG() {
  // Parabola opening upward with vertex marked
  const pts: string[] = [];
  for (let i = 0; i <= 60; i++) { const x = 40 + i * 3; const t = (i - 30) / 12; pts.push(`${x},${130 - 90 * (1 - t * t)}`); }
  return (
    <svg viewBox="0 0 260 160" className="w-full max-w-sm mx-auto" aria-hidden>
      <line x1={40} y1={20} x2={40} y2={145} stroke="#94a3b8" strokeWidth={1.2} />
      ...
      {/* Vertex */}
      <circle cx={130} cy={40} r={5} fill="#f59e0b" opacity={0.8} />
      <text x={138} y={36} fontSize={10} fill="#f59e0b" fontFamily="sans-serif">קודקוד?</text>
      {/* Axis of symmetry */}
      <line x1={130} y1={25} x2={130} y2={140} stroke="#a78bfa" strokeWidth={1} strokeDasharray="4,3" opacity={0.5} />
    </svg>
  );
}

function MediumSVG() {
  // Parabola opening downward — max problem
  const pts: string[] = [];
  for (let i = 0; i <= 60; i++) { const x = 30 + i * 3.3; const t = (i - 30) / 14; pts.push(`${x},${40 + 80 * t * t}`); }
  return (
    <svg viewBox="0 0 260 160" className="w-full max-w-sm mx-auto" aria-hidden>
      <line x1={40} y1={20} x2={40} y2={145} stroke="#94a3b8" strokeWidth={1.2} />
      <line x1={30} y1={130} x2={240} y2={130} stroke="#94a3b8" strokeWidth={1.2} />
      ...
      {/* Max vertex */}
      <circle cx={129} cy={40} r={5} fill="#34d399" />
      <text x={137} y={35} fontSize={10} fill="#34d399" fontFamily="sans-serif">max?</text>
      {/* Roots */}
      <circle cx={63} cy={130} r={3} fill="#64748b" />
      <circle cx={195} cy={130} r={3} fill="#64748b" />
    </svg>
  );
}

function AdvancedSVG() {
  // Rectangle under parabola — optimization problem
  const pts: string[] = [];
  for (let i = 0; i <= 60; i++) { const x = 30 + i * 3.3; const t = (i - 30) / 14; pts.push(`${x},${40 + 80 * t * t}`); }
  return (
    <svg viewBox="0 0 260 170" className="w-full max-w-sm mx-auto" aria-hidden>
      ...
      <polyline points={pts.join(" ")} fill="none" stroke="#DC2626" strokeWidth={2} opacity={0.6} />
      {/* Rectangle inscribed */}
      <rect x={80} y={65} width={100} height={75} fill="rgba(167,139,250,0.1)" stroke="#a78bfa" strokeWidth={1.5} strokeDasharray="4,3" rx={2} />
      <text x={130} y={108} fontSize={12} fill="#a78bfa" textAnchor="middle" fontFamily="sans-serif">S = ?</text>
      {/* Dimension labels */}
      <text x={130} y={155} fontSize={9} fill="#64748b" textAnchor="middle" fontFamily="sans-serif">רוחב?</text>
      <text x={185} y={108} fontSize={9} fill="#64748b" textAnchor="start" fontFamily="sans-serif">גובה?</text>
    </svg>
  );
}
...
const exercises: ExerciseDef[] = [
  {
    id: "basic",
    title: "קודקוד פרבולה — מקסימום או מינימום",
    problem: "נתונה הפונקציה f(x) = ax² + bx + c.\n\nא. הסבירו: מתי לפרבולה יש מינימום ומתי מקסימום (לפי סימן a)?\nב. מצאו את x של הקודקוד בעזרת הנוסחה x = −b/(2a).\nג. חשבו את ערך הקיצון y = f(x_קודקוד).",
    diagram: <BasicSVG />,
    pitfalls: [
      { title: "⚠️ בלבול בסימן a ובכיוון הפתיחה", text: "a > 0 → פתוחה למעלה → מינימום בקודקוד. a < 0 → פתוחה למטה → מקסימום בקודקוד. תלמידים מתבלבלים ואומרים שa>0 נותן מקסימום — ההפך." },
      { title: "⚠️ שכחת המינוס בנוסחת הקודקוד", text: "x_קודקוד = −b/(2a), לא b/(2a). שכחת המינוס גורמת לקודקוד בצד הלא נכון של ציר x." },
    ],
    goldenPrompt: `היי, אני תלמיד/ה כיתה יב', 3 יחידות, ומצרף/ת שאלה על קודקוד פרבולה — מקסימום ומינימום. אני רוצה שתהיה המורה הפרטי שלי — תעזור לי להבין ולא לתת תשובות ישירות.

אל תפתור עבורי — שאל אותי שאלות מכווינות.
סרוק את הנתונים בלבד.
אל תמהר, תסביר לי על כל שלב. בסיום הסריקה של הנתונים שהדבקתי, תגיב אך ורק: ״אני מוכן להמשיך.״`,
    steps: [
      { phase:"סעיף א׳", label:"סימן a וכיוון", coaching:"", prompt:"f(x) = ax² + bx + c. תנחה אותי — מתי a>0 ומתי a<0, ואיך זה קובע אם יש מקסימום או מינימום.", keywords:[], keywordHint:"", contextWords:["סימן","a","מקסימום","מינימום","פתוחה","כיוון"] },
      { phase:"סעיף ב׳", label:"x של הקודקוד", coaching:"", prompt:"תכווין אותי — מהי הנוסחה x = −b/(2a) ואיך מציבים ערכים מספריים.", keywords:[], keywordHint:"", contextWords:["קודקוד","x","נוסחה","b","a","מינוס"] },
      { phase:"סעיף ג׳", label:"ערך הקיצון", coaching:"", prompt:"מצאנו x של הקודקוד. תדריך אותי להציב ב-f(x) כדי למצוא את ערך y — זהו ערך הקיצון.", keywords:[], keywordHint:"", contextWords:["ערך","קיצון","הצבה","f(x)","y","קודקוד"] },
    ],
  },
  {
    id: "medium",
    title: "מציאת מקסימום — שורשים וקודקוד",
    problem: "כדור נזרק כלפי מעלה. גובהו לאחר t שניות: h(t) = −5t² + 30t + 10.\n\nא. מתי הכדור מגיע לגובה המרבי?\nב. מהו הגובה המרבי?\nג. מתי הכדור נוחת על הקרקע (h = 0)?",
    diagram: <MediumSVG />,
    pitfalls: [
      { title: "⚠️ בלבול בין זמן לגובה", text: "x_קודקוד = −b/(2a) נותן את הזמן (t). כדי למצוא את הגובה, חייבים להציב t זה חזרה ב-h(t). תלמידים כותבים את הזמן כתשובה לגובה." },
      { title: "⚠️ שכחת שיש שני שורשים", text: "h(t) = 0 נותן שני שורשים — אחד חיובי ואחד שלילי. בבעיה פיזיקלית, רק השורש החיובי רלוונטי (אי אפשר זמן שלילי)." },
    ],
    goldenPrompt: `אני בכיתה יב', 3 יחידות, מצרף/ת בעיית קיצון — כדור שנזרק, גובה מרבי וזמן נחיתה.

אל תיתן לי את החישוב — שאל אותי שאלות מנחות על מציאת הקודקוד.
סרוק את הנתונים בלבד.
אל תמהר, תסביר לי על כל שלב. בסיום הסריקה של הנתונים שהדבקתי, תגיב אך ורק: ״אני מוכן להמשיך.״`,
    steps: [
      { phase:"סעיף א׳", label:"זמן הגובה המרבי", coaching:"", prompt:"h(t) = −5t² + 30t + 10. a=−5, b=30. תנחה אותי — למה a<0 אומר מקסימום, ואיך מוצאים t_קודקוד.", keywords:[], keywordHint:"", contextWords:["קודקוד","זמן","t","מקסימום","נוסחה","a"] },
      { phase:"סעיף ב׳", label:"הגובה המרבי", coaching:"", prompt:"מצאנו t של הגובה המרבי. תכווין אותי להציב ב-h(t) ולמצוא את הגובה.", keywords:[], keywordHint:"", contextWords:["גובה","הצבה","מרבי","h(t)","ערך","חישוב"] },
      { phase:"סעיף ג׳", label:"זמן נחיתה", coaching:"", prompt:"h(t) = 0 → −5t² + 30t + 10 = 0. תדריך אותי — איך פותרים בנוסחת השורשים ולמה בוחרים את השורש החיובי.", keywords:[], keywordHint:"", contextWords:["שורש","נוסחה","דיסקרימיננטה","חיובי","h=0","נחיתה"] },
    ],
  },
  {
    id: "advanced",
    title: "בעיית קיצון מילולית — שטח מרבי",
    problem: "חקלאי רוצה לגדר שטח מלבני לאורך נהר (צד אחד לא צריך גדר).\nיש לו 60 מטר של גדר.\n\nא. הגדירו x כאורך הצלע הניצבת לנהר. כתבו ביטוי לאורך הצלע המקבילה לנהר.\nב. כתבו את פונקציית השטח S(x) ופתחו.\nג. מצאו את x שנותן שטח מרבי.\nד. מהו השטח המרבי?",
    diagram: <AdvancedSVG />,
    pitfalls: [
      { title: "⚠️ שכחת שצד אחד לא צריך גדר", text: "אם הנהר מהווה צד אחד, אז הגדר מכסה רק 3 צדדים: 2 ניצבים (x כל אחד) + 1 מקביל (60−2x). תלמידים כותבים 4 צדדים ומקבלים ביטוי שגוי." },
      { title: "⚠️ שכחה לבדוק תחום", text: "x חייב להיות חיובי ו-60−2x > 0, כלומר 0 < x < 30. הקודקוד חייב להיות בתחום — אם הוא מחוץ, הקיצון על שפת התחום." },
    ],
    goldenPrompt: "",
    advancedGateQuestion: "לפני שמתחילים — כתוב פרומפט שמסביר: מהם שלבי פתרון בעיית קיצון מילולית (משתנה → ביטוי → פונקציה → קודקוד → תשובה)? (לפחות 80 תווים)",
    steps: [
      { phase:"סעיף א׳", label:"הגדרת x וביטוי", coaching:"", prompt:"60 מ' גדר, 3 צדדים (2 ניצבים + 1 מקביל). x = ניצב. תנחה אותי — כמה גדר נשאר לצד המקביל.", keywords:[], keywordHint:"", contextWords:["משתנה","x","ביטוי","גדר","צד","ניצב"] },
      { phase:"סעיף ב׳", label:"פונקציית שטח", coaching:"", prompt:"שטח מלבן = אורך × רוחב = x × (60−2x). תכווין אותי לפתוח ולכתוב כפונקציה ריבועית.", keywords:[], keywordHint:"", contextWords:["שטח","פונקציה","ריבועית","פתיחה","S(x)","כפל"] },
      { phase:"סעיף ג׳", label:"x אופטימלי", coaching:"", prompt:"S(x) = −2x² + 60x. תדריך אותי — למה a<0 אומר מקסימום, ומצאו x_קודקוד.", keywords:[], keywordHint:"", contextWords:["קודקוד","מקסימום","x","a","נוסחה","אופטימלי"] },
      { phase:"סעיף ד׳", label:"שטח מרבי", coaching:"", prompt:"מצאנו x אופטימלי. תנחה אותי להציב ב-S(x) ולמצוא את השטח המרבי.", keywords:[], keywordHint:"", contextWords:["שטח","מרבי","הצבה","S","תשובה","ערך"] },
    ],
  },
];
  ...
  return (
    <section style={{ border:`1px solid ${s.glowBorder}`, borderRadius:24, padding:"2.5rem", background:"rgba(255,255,255,0.82)", backdropFilter:"blur(8px)", boxShadow:"0 10px 15px -3px rgba(60,54,42,0.1)" }}>
      <div style={{ borderRadius:16, border:`1px solid ${s.glowBorder}`, background:"rgba(255,255,255,0.75)", padding:"1.25rem 1.5rem", marginBottom:"2rem", boxShadow:s.glowShadow }}>
        <div style={{ textAlign:"center", marginBottom:14 }}><div style={{ color:s.accentColor, fontSize:15, fontWeight:800, marginBottom:4 }}>📈 קיצון פרבולה</div><div style={{ color:"#6B7280", fontSize:12, lineHeight:1.55 }}>{ex.id==="basic"&&"קודקוד פרבולה — כיוון, x ו-y של הקיצון."}{ex.id==="medium"&&"בעיית מקסימום — גובה כדור, שורשים."}{ex.id==="advanced"&&"בעיית קיצון מילולית — שטח מרבי."}</div></div>
        <div style={{ height:1, background:"rgba(60,54,42,0.1)", marginBottom:14 }}/>
        <div style={{ marginBottom:12 }}><div style={{ color:"#1A1A1A", fontSize:12, fontWeight:700, marginBottom:6 }}>🎯 נוסחאות קיצון</div><div style={{ display:"flex", flexDirection:"column", gap:5 }}><div style={{ display:"flex", alignItems:"baseline", gap:8, fontSize:13, color:"#2D3436", lineHeight:1.55 }}><span style={{ color:s.accentColor, fontFamily:"monospace", fontWeight:700, minWidth:120 }}>x = −b/(2a)</span><span>x של הקודקוד</span></div><div style={{ display:"flex", alignItems:"baseline", gap:8, fontSize:13, color:"#2D3436", lineHeight:1.55 }}><span style={{ color:s.accentColor, fontFamily:"monospace", fontWeight:700, minWidth:120 }}>a &gt; 0</span><span>מינימום (פתוחה למעלה)</span></div><div style={{ display:"flex", alignItems:"baseline", gap:8, fontSize:13, color:"#2D3436", lineHeight:1.55 }}><span style={{ color:s.accentColor, fontFamily:"monospace", fontWeight:700, minWidth:120 }}>a &lt; 0</span><span>מקסימום (פתוחה למטה)</span></div></div></div>
        {ex.id!=="basic"&&(<><div style={{ height:1, background:"rgba(60,54,42,0.08)", marginBottom:12 }}/><div style={{ marginBottom:12 }}><div style={{ color:"#1A1A1A", fontSize:12, fontWeight:700, marginBottom:6 }}>✨ {ex.id==="medium"?"נוסחת שורשים":"שלבי קיצון מילולי"}</div><div style={{ display:"flex", flexDirection:"column", gap:5 }}>{ex.id==="medium"&&<div style={{ display:"flex", alignItems:"baseline", gap:8, fontSize:13, color:"#2D3436", lineHeight:1.55 }}><span style={{ color:s.accentColor, fontFamily:"monospace", fontWeight:700, minWidth:140 }}>x = (−b±√Δ)/(2a)</span><span>Δ = b²−4ac</span></div>}{ex.id==="advanced"&&<><div style={{ display:"flex", alignItems:"baseline", gap:8, fontSize:13, color:"#2D3436", lineHeight:1.55 }}><span style={{ color:s.accentColor, fontFamily:"monospace", fontWeight:700, minWidth:30 }}>1.</span><span>הגדרת משתנה x</span></div><div style={{ display:"flex", alignItems:"baseline", gap:8, fontSize:13, color:"#2D3436", lineHeight:1.55 }}><span style={{ color:s.accentColor, fontFamily:"monospace", fontWeight:700, minWidth:30 }}>2.</span><span>כתיבת S(x) כפונקציה ריבועית</span></div><div style={{ display:"flex", alignItems:"baseline", gap:8, fontSize:13, color:"#2D3436", lineHeight:1.55 }}><span style={{ color:s.accentColor, fontFamily:"monospace", fontWeight:700, minWidth:30 }}>3.</span><span>קודקוד → ערך מרבי/מזערי</span></div></>}</div></div></>)}
      </div>
...
function VertexLab() {
  const [a,setA]=useState(-1); const [b,setB]=useState(6); const [c,setC]=useState(2);
  const st=STATION.basic;
  const xV=a!==0?-b/(2*a):0; const yV=a*xV*xV+b*xV+c;
  const type=a>0?"מינימום":a<0?"מקסימום":"ישר (לא פרבולה)";
  const f=(x:number)=>a*x*x+b*x+c;
  const ox=130,oy=120,sc=14;
  const toSx=(x:number)=>ox+x*sc; const toSy=(y:number)=>oy-y*sc;
  const pts:string[]=[];for(let x=-6;x<=10;x+=0.3){const y=f(x);if(y>-8&&y<15) pts.push(`${toSx(x)},${toSy(y)}`);}
  return (
    <section style={{ border:`1px solid ${st.glowBorder}`, borderRadius:24, padding:"2.5rem", background:"rgba(255,255,255,0.82)", backdropFilter:"blur(8px)", boxShadow:"0 10px 15px -3px rgba(60,54,42,0.1)", marginTop:"2rem" }}>
      ...
      <div style={{ borderRadius:16, border:`1px solid ${st.glowBorder}`, background:"rgba(255,255,255,0.75)", padding:"1.5rem", display:"flex", alignItems:"center", justifyContent:"center", marginBottom:"2rem", boxShadow:st.glowShadow }}>
        <svg viewBox="0 0 260 180" className="w-full max-w-md mx-auto" aria-hidden>
          <line x1={toSx(-6)} y1={oy} x2={toSx(10)} y2={oy} stroke="#94a3b8" strokeWidth={1}/><line x1={ox} y1={toSy(-8)} x2={ox} y2={toSy(15)} stroke="#94a3b8" strokeWidth={1}/>
          {pts.length>1&&<polyline points={pts.join(" ")} fill="none" stroke={st.accentColor} strokeWidth={2.5}/>}
          {a!==0&&<><circle cx={toSx(xV)} cy={toSy(yV)} r={6} fill="#f59e0b"/><text x={toSx(xV)+8} y={toSy(yV)-6} fontSize={9} fill="#f59e0b" fontFamily="monospace">({xV.toFixed(1)},{yV.toFixed(1)})</text><line x1={toSx(xV)} y1={toSy(-8)} x2={toSx(xV)} y2={toSy(15)} stroke="#a78bfa" strokeWidth={0.8} strokeDasharray="3,3" opacity={0.4}/></>}
        </svg>
      </div>
      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr 1fr", gap:8, textAlign:"center" }}>
        {[{label:"x קודקוד",val:a!==0?xV.toFixed(2):"—"},{label:"y קודקוד",val:a!==0?yV.toFixed(2):"—"},{label:"סוג",val:type},{label:"f(x)",val:`${a}x²+${b}x+${c}`}].map(r=>(<div key={r.label} style={{ borderRadius:16, background:"rgba(255,255,255,0.75)", border:`1px solid rgba(${st.glowRgb},0.4)`, padding:10, boxShadow:"0 4px 16px rgba(60,54,42,0.06)" }}><div style={{ color:"#6B7280", fontSize:10, fontWeight:600, marginBottom:4 }}>{r.label}</div><div style={{ color:st.accentColor, fontWeight:700, fontSize:14, fontFamily:"monospace" }}>{r.val}</div></div>))}
      </div>
      <p style={{ color:"#6B7280", fontSize:12, textAlign:"center", marginTop:16 }}>החליפו את סימן a — הפרבולה מתהפכת! מינימום ↔ מקסימום.</p>
    </section>
  );
}

/* ─── ProjectileLab (medium) ───────────────────────────────────────────────── */

function ProjectileLab() {
  const [v0,setV0]=useState(30); const [h0,setH0]=useState(10);
  const st=STATION.medium;
  const a=-5;
  const tMax=-v0/(2*a); const hMax=a*tMax*tMax+v0*tMax+h0;
  const disc=v0*v0-4*a*h0; const tLand=disc>=0?(-v0-Math.sqrt(disc))/(2*a):0;
  const f=(t:number)=>a*t*t+v0*t+h0;
  const ox2=40,oy2=150,scX=18,scY=1.5;
  const pts2:string[]=[];for(let t=0;t<=Math.max(tLand+0.5,8);t+=0.1){const h2=f(t);if(h2>=-10) pts2.push(`${ox2+t*scX},${oy2-h2*scY}`);}
  return (
    <section style={{ border:`1px solid ${st.glowBorder}`, borderRadius:24, padding:"2.5rem", background:"rgba(255,255,255,0.82)", backdropFilter:"blur(8px)", boxShadow:"0 10px 15px -3px rgba(60,54,42,0.1)", marginTop:"2rem" }}>
      <h3 style={{ color:"#2D3436", fontSize:22, fontWeight:800, textAlign:"center", marginBottom:8 }}>מעבדת זריקת כדור</h3>
      <p style={{ color:"#6B7280", fontSize:14, textAlign:"center", marginBottom:"2rem" }}>שנו מהירות התחלתית וגובה — צפו בגובה מרבי ובזמן נחיתה.</p>
      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:12, maxWidth:400, margin:"0 auto 2rem" }}>
        {[{label:"v₀ (מ/ש)",val:v0,set:setV0,min:10,max:50,step:5},{label:"h₀ (מטר)",val:h0,set:setH0,min:0,max:30,step:5}].map((s,i)=>(<div key={i} style={{ background:"rgba(255,255,255,0.75)", borderRadius:16, border:"1px solid rgba(255,255,255,0.4)", padding:"1.25rem", boxShadow:"0 4px 16px rgba(60,54,42,0.12)" }}><div style={{ display:"flex", justifyContent:"space-between", fontSize:12, color:"#6B7280", marginBottom:4 }}><span>{s.label}</span><span style={{ color:st.accentColor, fontFamily:"monospace", fontWeight:700 }}>{s.val}</span></div><input type="range" min={s.min} max={s.max} step={s.step} value={s.val} onChange={e=>s.set(+e.target.value)} style={{ width:"100%", accentColor:st.accentColor }}/></div>))}
      </div>
      <div style={{ borderRadius:16, border:`1px solid ${st.glowBorder}`, background:"rgba(255,255,255,0.75)", padding:"1.5rem", display:"flex", alignItems:"center", justifyContent:"center", marginBottom:"2rem", boxShadow:st.glowShadow }}>
        <svg viewBox="0 0 260 170" className="w-full max-w-md mx-auto" aria-hidden>
          <line x1={ox2} y1={oy2} x2={250} y2={oy2} stroke="#94a3b8" strokeWidth={1}/><line x1={ox2} y1={10} x2={ox2} y2={oy2+5} stroke="#94a3b8" strokeWidth={1}/>
          {pts2.length>1&&<polyline points={pts2.join(" ")} fill="none" stroke={st.accentColor} strokeWidth={2.5}/>}
          <circle cx={ox2+tMax*scX} cy={oy2-hMax*scY} r={5} fill="#34d399"/>
          <text x={ox2+tMax*scX+6} y={oy2-hMax*scY-6} fontSize={9} fill="#34d399" fontFamily="monospace">max</text>
          {tLand>0&&<circle cx={ox2+tLand*scX} cy={oy2} r={4} fill="#DC2626" opacity={0.7}/>}
        </svg>
      </div>
      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr 1fr", gap:8, textAlign:"center" }}>
        {[{label:"t(max)",val:tMax.toFixed(1)+"s"},{label:"h(max)",val:hMax.toFixed(1)+"m"},{label:"t(land)",val:tLand>0?tLand.toFixed(1)+"s":"—"},{label:"h(t)",val:`−5t²+${v0}t+${h0}`}].map(r=>(<div key={r.label} style={{ borderRadius:16, background:"rgba(255,255,255,0.75)", border:`1px solid rgba(${st.glowRgb},0.4)`, padding:10, boxShadow:"0 4px 16px rgba(60,54,42,0.06)" }}><div style={{ color:"#6B7280", fontSize:10, fontWeight:600, marginBottom:4 }}>{r.label}</div><div style={{ color:st.accentColor, fontWeight:700, fontSize:14, fontFamily:"monospace" }}>{r.val}</div></div>))}
      </div>
      <p style={{ color:"#6B7280", fontSize:12, textAlign:"center", marginTop:16 }}>הגדילו v₀ — הכדור עולה גבוה יותר ולוקח יותר זמן לנחות!</p>
    </section>
  );
}
...
function FenceLab() {
  const [fence,setFence]=useState(60);
  const st=STATION.advanced;
  const xOpt=fence/4; const maxArea=fence*fence/8;
  const f=(x:number)=>x*(fence-2*x);
  const ox3=40,oy3=140,scX3=4,scY3=0.25;
  const pts3:string[]=[];for(let x=0;x<=fence/2;x+=0.5){const y=f(x);pts3.push(`${ox3+x*scX3},${oy3-y*scY3}`);}
  return (
    <section style={{ border:`1px solid ${st.glowBorder}`, borderRadius:24, padding:"2.5rem", background:"rgba(255,255,255,0.82)", backdropFilter:"blur(8px)", boxShadow:"0 10px 15px -3px rgba(60,54,42,0.1)", marginTop:"2rem" }}>
      <h3 style={{ color:"#2D3436", fontSize:22, fontWeight:800, textAlign:"center", marginBottom:8 }}>מעבדת גידור — שטח מרבי</h3>
      <p style={{ color:"#6B7280", fontSize:14, textAlign:"center", marginBottom:"2rem" }}>שנו את אורך הגדר — צפו כיצד x האופטימלי והשטח המרבי משתנים.</p>
      <div style={{ maxWidth:400, margin:"0 auto 2rem" }}><div style={{ background:"rgba(255,255,255,0.75)", borderRadius:16, border:"1px solid rgba(255,255,255,0.4)", padding:"1.25rem", boxShadow:"0 4px 16px rgba(60,54,42,0.12)" }}><div style={{ display:"flex", justifyContent:"space-between", fontSize:12, color:"#6B7280", marginBottom:4 }}><span>אורך גדר (מ')</span><span style={{ color:st.accentColor, fontFamily:"monospace", fontWeight:700 }}>{fence}</span></div><input type="range" min={20} max={120} step={5} value={fence} onChange={e=>setFence(+e.target.value)} style={{ width:"100%", accentColor:st.accentColor }}/></div></div>
      <div style={{ borderRadius:16, border:`1px solid ${st.glowBorder}`, background:"rgba(255,255,255,0.75)", padding:"1.5rem", display:"flex", alignItems:"center", justifyContent:"center", marginBottom:"2rem", boxShadow:st.glowShadow }}>
        <svg viewBox="0 0 260 160" className="w-full max-w-md mx-auto" aria-hidden>
          <line x1={ox3} y1={oy3} x2={250} y2={oy3} stroke="#94a3b8" strokeWidth={1}/><line x1={ox3} y1={10} x2={ox3} y2={oy3+5} stroke="#94a3b8" strokeWidth={1}/>
          {pts3.length>1&&<polyline points={pts3.join(" ")} fill="none" stroke={st.accentColor} strokeWidth={2.5}/>}
          <circle cx={ox3+xOpt*scX3} cy={oy3-maxArea*scY3} r={5} fill="#34d399"/>
          <text x={ox3+xOpt*scX3+6} y={oy3-maxArea*scY3-6} fontSize={9} fill="#34d399" fontFamily="monospace">max</text>
          <text x={ox3+xOpt*scX3} y={oy3+14} fontSize={9} fill="#f59e0b" textAnchor="middle" fontFamily="monospace">x={xOpt}</text>
        </svg>
      </div>
      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr 1fr", gap:8, textAlign:"center" }}>
        {[{label:"גדר",val:`${fence} מ'`},{label:"x אופטימלי",val:`${xOpt} מ'`},{label:"צד מקביל",val:`${fence-2*xOpt} מ'`},{label:"שטח מרבי",val:`${maxArea} מ"ר`}].map(r=>(<div key={r.label} style={{ borderRadius:16, background:"rgba(255,255,255,0.75)", border:`1px solid rgba(${st.glowRgb},0.4)`, padding:10, boxShadow:"0 4px 16px rgba(60,54,42,0.06)" }}><div style={{ color:"#6B7280", fontSize:10, fontWeight:600, marginBottom:4 }}>{r.label}</div><div style={{ color:st.accentColor, fontWeight:700, fontSize:15, fontFamily:"monospace" }}>{r.val}</div></div>))}
      </div>
      <p style={{ color:"#6B7280", fontSize:12, textAlign:"center", marginTop:16 }}>x תמיד = רבע הגדר, והצד המקביל = חצי הגדר. למה?</p>
    </section>
  );
}
...
function FormulaBar() {
  const [activeTab,setActiveTab]=useState<"vertex"|"roots"|"method"|null>(null);
  const tabs=[{id:"vertex" as const,label:"📐 קודקוד",tex:"x = \\frac{-b}{2a}",color:"#16A34A",borderColor:"rgba(22,163,74,0.35)"},{id:"roots" as const,label:"📏 שורשים",tex:"\\Delta = b^2-4ac",color:"#EA580C",borderColor:"rgba(234,88,12,0.35)"},{id:"method" as const,label:"📋 שיטת קיצון",tex:"S_{\\max}",color:"#DC2626",borderColor:"rgba(220,38,38,0.35)"}];
  return (
    <div style={{ borderRadius:12, border:"1px solid rgba(60,54,42,0.15)", background:"rgba(255,255,255,0.75)", padding:"1.25rem", marginBottom:"1.25rem", boxShadow:"0 4px 16px rgba(60,54,42,0.08)" }}>
      ...
      {activeTab==="vertex"&&(<motion.div key="v" initial={{opacity:0,height:0}} animate={{opacity:1,height:"auto"}} style={{overflow:"hidden"}}><div style={{ borderRadius:12, border:"2px solid rgba(22,163,74,0.25)", background:"rgba(220,252,231,0.4)", padding:16 }}><div dir="ltr" style={{ textAlign:"center", marginBottom:14 }}><DisplayMath>{"x_v = \\frac{-b}{2a}, \\quad y_v = f(x_v)"}</DisplayMath></div><div style={{ borderRadius:10, background:"rgba(22,163,74,0.06)", border:"1px solid rgba(22,163,74,0.15)", padding:"12px 14px" }}><div style={{ color:"#1A1A1A", fontSize:12, lineHeight:2, fontWeight:500 }}><strong>קודקוד = ערך קיצון</strong><ol dir="rtl" style={{ margin:"6px 0 0", paddingInlineStart:18 }}><li>a &gt; 0 → מינימום (U). a &lt; 0 → מקסימום (∩).</li><li>ציר הסימטריה: x = −b/(2a).</li><li>ערך הקיצון: הציבו x_v ב-f(x).</li></ol></div></div></div></motion.div>)}
      {activeTab==="roots"&&(<motion.div key="r" initial={{opacity:0,height:0}} animate={{opacity:1,height:"auto"}} style={{overflow:"hidden"}}><div style={{ borderRadius:12, border:"2px solid rgba(234,88,12,0.25)", background:"rgba(255,247,237,0.95)", padding:16 }}><div dir="ltr" style={{ textAlign:"center", marginBottom:14 }}><DisplayMath>{"x = \\frac{-b \\pm \\sqrt{b^2-4ac}}{2a}"}</DisplayMath></div><div style={{ borderRadius:10, background:"rgba(234,88,12,0.06)", border:"1px solid rgba(234,88,12,0.15)", padding:"12px 14px" }}><div style={{ color:"#1A1A1A", fontSize:12, lineHeight:2, fontWeight:500 }}><strong>נוסחת השורשים</strong><ol dir="rtl" style={{ margin:"6px 0 0", paddingInlineStart:18 }}><li>Δ &gt; 0 → שני שורשים שונים.</li><li>Δ = 0 → שורש כפול (הקודקוד על ציר x).</li><li>Δ &lt; 0 → אין שורשים ממשיים.</li></ol></div></div></div></motion.div>)}
      {activeTab==="method"&&(<motion.div key="m" initial={{opacity:0,height:0}} animate={{opacity:1,height:"auto"}} style={{overflow:"hidden"}}><div style={{ borderRadius:12, border:"2px solid rgba(220,38,38,0.25)", background:"rgba(254,242,242,0.95)", padding:16 }}><div style={{ borderRadius:10, background:"rgba(220,38,38,0.06)", border:"1px solid rgba(220,38,38,0.15)", padding:"12px 14px" }}><div style={{ color:"#1A1A1A", fontSize:12, lineHeight:2, fontWeight:500 }}><strong>שלבי פתרון בעיית קיצון</strong><ol dir="rtl" style={{ margin:"6px 0 0", paddingInlineStart:18 }}><li>הגדירו משתנה x (מה משתנה בבעיה).</li><li>בטאו את הגודל לאופטימיזציה כ-f(x).</li><li>פתחו ל-ax² + bx + c.</li><li>מצאו x_קודקוד = −b/(2a).</li><li>הציבו → ערך הקיצון.</li><li>בדקו: x בתחום? a &gt; 0 → min, a &lt; 0 → max.</li></ol></div><div style={{ marginTop:10, color:"#DC2626", fontSize:11, fontWeight:600, lineHeight:1.7 }}>💡 דוגמה: גדר 60 מ', S(x) = x(60−2x) = −2x²+60x → x=15, S=450</div></div></div></motion.div>)}
    </div>
  );
}
...
export default function ExtremumPage() {
    ...
    <main style={{ minHeight:"100vh", background:"#F3EFE0", backgroundImage:"radial-gradient(rgba(60,54,42,0.07) 1px, transparent 1px)", backgroundSize:"24px 24px", color:"#2D3436", ["--lvl-rgb" as string]:lvlRgb } as React.CSSProperties} dir="rtl">
      ...
      <div style={{ borderBottom:"1px solid rgba(60,54,42,0.15)", background:"#F3EFE0" }}>
        <div style={{ maxWidth:"56rem", margin:"0 auto", padding:"0.9rem 1.5rem", display:"flex", alignItems:"center", justifyContent:"space-between", gap:"1rem" }}>
          <div><h1 style={{ fontSize:22, fontWeight:700, color:"#2D3436", margin:0 }}>קיצון פרבולה עם AI — כיתה יב׳ (3 יח׳)</h1><p style={{ fontSize:13, color:"#6B7280", margin:"2px 0 0" }}>קודקוד, מקסימום/מינימום, בעיות קיצון מילוליות</p></div>
          <Link href="/3u/topic/grade12/parabola" style={{ display:"flex", alignItems:"center", gap:6, padding:"8px 14px", background:"#4A4A4A", border:"1px solid #333", borderRadius:10, fontSize:14, fontWeight:600, color:"#FFFFFF", textDecoration:"none", whiteSpace:"nowrap" }} onMouseEnter={e=>{(e.currentTarget as HTMLAnchorElement).style.background="#2D2D2D";}} onMouseLeave={e=>{(e.currentTarget as HTMLAnchorElement).style.background="#4A4A4A";}}><span style={{ fontSize:16 }}>←</span>חזרה</Link>
        </div>
      </div>
      <div style={{ maxWidth:"56rem", margin:"0 auto", padding:"2rem 1rem 5rem" }}>
        <SubtopicProgress subtopicId="3u/grade12/parabola/extremum"/>
        ...
        {selectedLevel==="basic"&&<VertexLab/>}
        {selectedLevel==="medium"&&<ProjectileLab/>}
        {selectedLevel==="advanced"&&<FenceLab/>}
        <div style={{ marginTop:"1.5rem" }}><MarkComplete subtopicId="3u/grade12/parabola/extremum" level={selectedLevel}/></div>
      </div>
    </main>
  );
}
