...
import { Brain, ChevronRight, Sigma, Search, BarChart3 } from "lucide-react";
...
const SUBTOPICS = [
  {
    ...
    description: "גזירת פונקציות רציונליות ושורש, מציאת תחום הגדרה, אסימפטוטות אנכיות ואופקיות.",
    chips: ["f(x) = P/Q", "√x", "אסימפטוטה"],
    ready: true,
    glowAnim: false,
  ...
  },
  {
    ...
    description: "חקירת פונקציה מלאה — תחום, נגזרת, קיצון, פיתול, תחומי עלייה/ירידה, קעירות ושרטוט גרף.",
    chips: ["f'(x) = 0", "f''(x)", "שרטוט גרף"],
    ready: true,
    glowAnim: false,
  ...
  {
    id: "integral",
    href: "/5u/topic/grade11/calculus/integral",
    icon: BarChart3,
    title: "אינטגרל ושטח",
    subtitle: "אינטגרל • שטח בין גרפים • נפח",
    description: "אינטגרל לא מסוים ומסוים, חישוב שטחים בין עקומות וציר ה-x, ושטח בין שני גרפים.",
    chips: ["∫f(x)dx", "שטח", "F(b)−F(a)"],
    ready: true,
    glowAnim: false,
...
export default function Calculus5UHub() {
  return (
    <div className="min-h-screen bg-[#0a0f1e] text-white" dir="rtl">
      ...
      <header className="bg-[#0f172a] border-b border-slate-800 sticky top-0 z-10">
        <div className="max-w-2xl mx-auto px-4 py-3 flex items-center justify-between">
          ...
          <Link href="/" className="text-slate-400 hover:text-slate-200 text-sm transition-colors flex items-center gap-1">
            <ChevronRight size={14} className="rotate-180" />כיתה י״א • 5 יח&quot;ל
          </Link>
        </div>
      </header>
      <main className="max-w-2xl mx-auto px-4 py-14 space-y-10">
        <div className="text-center space-y-3 animate-[fadeSlideIn_0.4s_ease_both]">
          <p className="text-blue-400 text-xs font-semibold uppercase tracking-widest">כיתה י״א • 5 יח&quot;ל</p>
          <h1 className="text-4xl font-extrabold">חדו״א ואינטגרל</h1>
          <p className="text-slate-400 text-sm">פונקציות רציונליות ושורש • חקירה מלאה • אינטגרל ושטח</p>
        </div>
