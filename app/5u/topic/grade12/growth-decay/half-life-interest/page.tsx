...
function BasicDiagram() {
  return (
    <div style={{ borderRadius: 12, background: "rgba(255,255,255,0.7)", border: "1px solid rgba(22,163,74,0.2)", padding: 12 }}>
      <p style={{ fontSize: 11, color: "#94a3b8", textAlign: "center", marginBottom: 4 }}>דעיכה מעריכית — חצי חיים</p>
      <svg width="100%" viewBox="0 0 280 160" style={{ maxWidth: "100%" }}>
        <line x1={40} y1={140} x2={260} y2={140} stroke="#cbd5e1" strokeWidth={1} />
        <line x1={40} y1={150} x2={40} y2={15} stroke="#cbd5e1" strokeWidth={1} />
        <text x={264} y={144} fill="#94a3b8" fontSize={10}>t</text>
        <text x={34} y={12} fill="#94a3b8" fontSize={10}>N</text>
        <path d="M 40,25 C 80,25 100,60 130,80 C 160,100 200,120 260,130" fill="none" stroke="#16a34a" strokeWidth={2} />
        <line x1={40} y1={25} x2={260} y2={25} stroke="#64748b" strokeWidth={0.7} strokeDasharray="3,3" />
        <text x={262} y={22} fill="#64748b" fontSize={9}>N\u2080</text>
        <line x1={40} y1={82} x2={130} y2={82} stroke="#f59e0b" strokeWidth={1} strokeDasharray="4,3" />
        <line x1={130} y1={82} x2={130} y2={140} stroke="#f59e0b" strokeWidth={1} strokeDasharray="4,3" />
        <text x={80} y={76} fill="#f59e0b" fontSize={9} fontWeight={600}>N\u2080/2</text>
        <text x={126} y={152} fill="#f59e0b" fontSize={9} fontWeight={600}>T\u00BD</text>
      </svg>
    </div>
  );
}
...
function MediumDiagram() {
  return (
    <div style={{ borderRadius: 12, background: "rgba(255,255,255,0.7)", border: "1px solid rgba(234,88,12,0.2)", padding: 12 }}>
      <p style={{ fontSize: 11, color: "#94a3b8", textAlign: "center", marginBottom: 4 }}>גדילה מעריכית — ריבית דריבית</p>
      <svg width="100%" viewBox="0 0 280 160" style={{ maxWidth: "100%" }}>
        <line x1={40} y1={140} x2={260} y2={140} stroke="#cbd5e1" strokeWidth={1} />
        <line x1={40} y1={150} x2={40} y2={15} stroke="#cbd5e1" strokeWidth={1} />
        <text x={264} y={144} fill="#94a3b8" fontSize={10}>t</text>
        <text x={34} y={12} fill="#94a3b8" fontSize={10}>A</text>
        <path d="M 40,130 C 80,125 120,110 160,85 C 200,55 240,25 260,15" fill="none" stroke="#ea580c" strokeWidth={2} />
        <circle cx={40} cy={130} r={3} fill="#ea580c" />
        <text x={26} y={128} fill="#ea580c" fontSize={9}>P</text>
        <line x1={40} y1={130} x2={260} y2={85} stroke="#64748b" strokeWidth={1} strokeDasharray="4,3" />
        <text x={200} y={80} fill="#64748b" fontSize={9}>ריבית פשוטה</text>
        <text x={220} y={30} fill="#ea580c" fontSize={9}>ריבית דריבית</text>
      </svg>
    </div>
  );
}
...
function AdvancedDiagram() {
  return (
    <div style={{ borderRadius: 12, background: "rgba(255,255,255,0.7)", border: "1px solid rgba(220,38,38,0.2)", padding: 12 }}>
      <p style={{ fontSize: 11, color: "#94a3b8", textAlign: "center", marginBottom: 4 }}>ריבית רציפה — e^rt</p>
      <svg width="100%" viewBox="0 0 280 160" style={{ maxWidth: "100%" }}>
        <line x1={40} y1={140} x2={260} y2={140} stroke="#cbd5e1" strokeWidth={1} />
        <line x1={40} y1={150} x2={40} y2={15} stroke="#cbd5e1" strokeWidth={1} />
        <text x={264} y={144} fill="#94a3b8" fontSize={10}>t</text>
        <text x={34} y={12} fill="#94a3b8" fontSize={10}>A</text>
        {/* Multiple curves for different compounding */}
        <path d="M 40,120 C 100,115 160,95 220,60 L 260,40" fill="none" stroke="#64748b" strokeWidth={1.3} strokeDasharray="4,3" />
        <path d="M 40,120 C 90,112 140,88 200,50 L 250,25" fill="none" stroke="#a78bfa" strokeWidth={1.5} />
        <path d="M 40,120 C 85,110 130,80 190,42 L 245,18" fill="none" stroke="#dc2626" strokeWidth={2} />
        <text x={230} y={55} fill="#64748b" fontSize={8}>שנתית</text>
        <text x={218} y={32} fill="#a78bfa" fontSize={8}>חודשית</text>
        <text x={210} y={15} fill="#dc2626" fontSize={8}>רציפה e\u02B3\u1D57</text>
      </svg>
    </div>
  );
}
...
function FormulaBar() {
  const [act, setAct] = useState<"half" | "compound" | "continuous" | "doubling" | null>(null);
  const tabs = [
    { id: "half" as const, label: "חצי חיים", tex: "T_{\\frac{1}{2}}=\\frac{\\ln 2}{k}", color: "#16A34A", bc: "rgba(22,163,74,0.35)" },
    { id: "compound" as const, label: "ריבית דריבית", tex: "A=P(1+\\frac{r}{n})^{nt}", color: "#EA580C", bc: "rgba(234,88,12,0.35)" },
    { id: "continuous" as const, label: "ריבית רציפה", tex: "A=Pe^{rt}", color: "#DC2626", bc: "rgba(220,38,38,0.35)" },
    { id: "doubling" as const, label: "זמן הכפלה", tex: "T_2=\\frac{\\ln 2}{r}", color: "#7c3aed", bc: "rgba(124,58,237,0.35)" },
  ];
  return (
    <div style={{ borderRadius: 12, border: "1px solid rgba(60,54,42,0.15)", background: "rgba(255,255,255,0.82)", padding: "1.25rem", marginBottom: "1.25rem" }}>
      ...
      <div style={{ display: "flex", gap: 6, marginBottom: act ? 14 : 0 }}>
        {tabs.map(t => { const a = act === t.id; return <button key={t.id} onClick={() => setAct(a ? null : t.id)} style={{ flex: 1, padding: "10px 4px", borderRadius: 10, cursor: "pointer", transition: "all 0.2s", border: `1.5px solid ${a ? t.bc : "rgba(60,54,42,0.15)"}`, background: a ? `${t.color}15` : "rgba(255,255,255,0.5)", display: "flex", flexDirection: "column", alignItems: "center", gap: 4 }}><span style={{ fontSize: 10, fontWeight: 700, color: a ? t.color : "#6B7280" }}>{t.label}</span><span style={{ color: a ? t.color : "#6B7280" }}><InlineMath>{t.tex}</InlineMath></span></button>; })}
      </div>
      {act === "half" && <motion.div key="half" initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} style={{ overflow: "hidden" }}><div style={{ borderRadius: 12, border: "2px solid rgba(22,163,74,0.25)", background: "rgba(22,163,74,0.06)", padding: 16 }}><div dir="ltr" style={{ textAlign: "center", marginBottom: 14 }}><DisplayMath>{"N(t) = N_0 \\cdot e^{-kt},\\quad T_{1/2} = \\frac{\\ln 2}{k}"}</DisplayMath></div><div style={{ borderRadius: 10, background: "rgba(22,163,74,0.08)", border: "1px solid rgba(22,163,74,0.15)", padding: "12px 14px", color: "#2D3436", fontSize: 12, lineHeight: 2, fontWeight: 500 }}><strong>חצי חיים:</strong> הזמן שלוקח לחומר להתפרק למחצית. k = קצב הדעיכה. אם ידוע T\u00BD: k = ln2/T\u00BD.</div></div></motion.div>}
      {act === "compound" && <motion.div key="compound" initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} style={{ overflow: "hidden" }}><div style={{ borderRadius: 12, border: "2px solid rgba(234,88,12,0.25)", background: "rgba(234,88,12,0.06)", padding: 16 }}><div dir="ltr" style={{ textAlign: "center", marginBottom: 14 }}><DisplayMath>{"A = P\\left(1 + \\frac{r}{n}\\right)^{nt}"}</DisplayMath></div><div style={{ borderRadius: 10, background: "rgba(234,88,12,0.08)", border: "1px solid rgba(234,88,12,0.15)", padding: "12px 14px", color: "#2D3436", fontSize: 12, lineHeight: 2, fontWeight: 500 }}><strong>ריבית דריבית:</strong> P = קרן, r = ריבית שנתית, n = מספר צבירות בשנה, t = שנים. n=1 שנתית, n=12 חודשית, n=365 יומית.</div></div></motion.div>}
      {act === "continuous" && <motion.div key="continuous" initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} style={{ overflow: "hidden" }}><div style={{ borderRadius: 12, border: "2px solid rgba(220,38,38,0.25)", background: "rgba(220,38,38,0.06)", padding: 16 }}><div dir="ltr" style={{ textAlign: "center", marginBottom: 14 }}><DisplayMath>{"A = P \\cdot e^{rt}"}</DisplayMath></div><div style={{ borderRadius: 10, background: "rgba(220,38,38,0.08)", border: "1px solid rgba(220,38,38,0.15)", padding: "12px 14px", color: "#2D3436", fontSize: 12, lineHeight: 2, fontWeight: 500 }}><strong>ריבית רציפה:</strong> כש-n\u2192\u221E, מגיעים ל-A = Pe\u02B3\u1D57. זהו הגבול של ריבית דריבית עם צבירה אינסופית.</div></div></motion.div>}
      {act === "doubling" && <motion.div key="doubling" initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} style={{ overflow: "hidden" }}><div style={{ borderRadius: 12, border: "2px solid rgba(124,58,237,0.25)", background: "rgba(124,58,237,0.06)", padding: 16 }}><div dir="ltr" style={{ textAlign: "center", marginBottom: 14 }}><DisplayMath>{"T_2 = \\frac{\\ln 2}{r} \\approx \\frac{0.693}{r}"}</DisplayMath></div><div style={{ borderRadius: 10, background: "rgba(124,58,237,0.08)", border: "1px solid rgba(124,58,237,0.15)", padding: "12px 14px", color: "#2D3436", fontSize: 12, lineHeight: 2, fontWeight: 500 }}><strong>זמן הכפלה:</strong> אחרי כמה שנים הכסף מוכפל? כלל 72: T\u2082 \u2248 72/r% (קירוב מהיר).</div></div></motion.div>}
    </div>
  );
}
...
const CW_B: string[] = ["חצי חיים", "דעיכה", "רדיואקטיבי", "ln", "k", "N\u2080"];
const CW_M: string[] = ["ריבית", "דריבית", "קרן", "שנתית", "הכפלה", "חודשית"];
const CW_A: string[] = ["רציפה", "e", "ln", "פחת", "מודל", "השוואה"];

const exercises: ExerciseDef[] = [
  {
    id: "basic", title: "חצי חיים — דעיכה רדיואקטיבית",
    problem: "חומר רדיואקטיבי עם זמן מחצית חיים T\u00BD = 8 שעות. בהתחלה יש 200 גרם.\n\nא. כתבו את נוסחת הכמות N(t).\nב. כמה יישאר אחרי 24 שעות?\nג. אחרי כמה שעות יישארו פחות מ-10 גרם?",
    diagram: <BasicDiagram />,
    pitfalls: [
      { title: "k = ln2/T\u00BD = ln2/8 \u2248 0.0866", text: "N(t) = 200\u00B7e^(\u22120.0866t) או N(t) = 200\u00B7(1/2)^(t/8)." },
      { title: "24 שעות = 3 חצאי חיים", text: "N(24) = 200\u00B7(1/2)\u00B3 = 200/8 = 25 גרם." },
      { title: "N(t) < 10: פתרו 200\u00B7(1/2)^(t/8) = 10", text: "(1/2)^(t/8) = 0.05 \u2192 t/8 = log\u2080.\u2085(0.05) = ln(0.05)/ln(0.5) \u2248 4.32 \u2192 t \u2248 34.6 שעות." },
    ],
    goldenPrompt: "אתה המורה הפרטי שלי, כיתה יב 5 יחידות.\nחומר רדיואקטיבי: T\u00BD=8 שעות, N\u2080=200 גרם.\nצריך: נוסחה, N(24), מתי N<10." + PF,
    steps: [
      { phase: "א", label: "נוסחה", coaching: "N=N\u2080\u00B7e^(\u2212kt)", prompt: "T\u00BD=8, N\u2080=200. הנחה אותי לכתוב N(t)." + PF, keywords: ["נוסחה", "k"], keywordHint: "ציין נוסחה", contextWords: CW_B },
      { phase: "ב", label: "N(24)", coaching: "3 חצאי חיים", prompt: "N(t) = 200\u00B7(1/2)^(t/8). הנחה אותי לחשב N(24)." + PF, keywords: ["הצבה", "24"], keywordHint: "ציין N(24)", contextWords: CW_B },
      { phase: "ג", label: "מתי N<10", coaching: "לוגריתם", prompt: "N(t)<10. הנחה אותי לפתור עם ln." + PF, keywords: ["ln", "פתרון"], keywordHint: "ציין מתי", contextWords: CW_B },
    ],
  },
  {
    id: "medium", title: "ריבית דריבית — חיסכון",
    problem: "הפקדת 50,000 \u20AA בריבית שנתית 6% עם צבירה חודשית.\n\nא. כתבו את הנוסחה לסכום A אחרי t שנים.\nב. כמה כסף יהיה אחרי 10 שנים?\nג. אחרי כמה שנים הכסף יוכפל?\nד. מה ההפרש בין צבירה חודשית לשנתית אחרי 10 שנים?",
    diagram: <MediumDiagram />,
    pitfalls: [
      { title: "A = 50000(1 + 0.06/12)^(12t)", text: "r=0.06, n=12 (חודשית). אל תשכחו לחלק ב-12!" },
      { title: "A(10) = 50000(1.005)\u00B9\u00B2\u2070 \u2248 90,970 \u20AA", text: "120 חודשים \u2014 לא 10." },
      { title: "הכפלה: (1.005)\u00B9\u00B2\u1D57 = 2 \u2192 12t\u00B7ln(1.005) = ln2", text: "t = ln2/(12\u00B7ln1.005) \u2248 11.58 שנים. כלל 72: 72/6 = 12 \u2248 דומה!" },
    ],
    goldenPrompt: "אתה המורה הפרטי שלי, כיתה יב 5 יחידות.\n50,000 \u20AA, ריבית 6% חודשית.\nצריך: נוסחה, A(10), זמן הכפלה, השוואה שנתית/חודשית." + PF,
    steps: [
      { phase: "א", label: "נוסחה", coaching: "P(1+r/n)^(nt)", prompt: "P=50000, r=6%, n=12. הנחה אותי לכתוב נוסחה." + PF, keywords: ["דריבית", "נוסחה"], keywordHint: "ציין נוסחה", contextWords: CW_M },
      { phase: "ב", label: "A(10)", coaching: "הציבו t=10", prompt: "A=50000(1.005)^(12t). הנחה אותי לחשב A(10)." + PF, keywords: ["הצבה", "10 שנים"], keywordHint: "ציין A(10)", contextWords: CW_M },
      { phase: "ג", label: "הכפלה", coaching: "A=2P \u2192 ln", prompt: "הנחה אותי למצוא t שבו הכסף מוכפל." + PF, keywords: ["הכפלה", "ln"], keywordHint: "ציין הכפלה", contextWords: CW_M },
      { phase: "ד", label: "השוואה", coaching: "n=1 מול n=12", prompt: "השוו A אחרי 10 שנים: שנתית (n=1) מול חודשית (n=12)." + PF, keywords: ["השוואה", "שנתית"], keywordHint: "ציין השוואה", contextWords: CW_M },
    ],
  },
  {
    id: "advanced", title: "ריבית רציפה ופחת",
    problem: "רכב נרכש ב-180,000 \u20AA. ערכו יורד ב-15% בשנה (פחת מעריכי). במקביל, חסכון בריבית רציפה של 5%.\n\nא. כתבו את נוסחת ערך הרכב V(t) ואת נוסחת החיסכון A(t) = 180000\u00B7e^(0.05t).\nב. אחרי כמה שנים ערך הרכב יירד מתחת ל-50,000?\nג. אחרי כמה שנים החיסכון יעלה על ערך הרכב?\nד. מהי הריבית האפקטיבית השנתית של 5% רציפה?",
    diagram: <AdvancedDiagram />,
    pitfalls: [
      { title: "V(t) = 180000\u00B7(0.85)\u1D57 = 180000\u00B7e^(\u22120.1625t)", text: "פחת 15% \u2192 נשאר 85% \u2192 q=0.85. k = \u2212ln(0.85) \u2248 0.1625." },
      { title: "V(t) < 50000: 0.85\u1D57 < 50/180 \u2248 0.278", text: "t > ln(0.278)/ln(0.85) \u2248 7.9 שנים." },
      { title: "A(t) > V(t): e^(0.05t) > (0.85)\u1D57", text: "e^(0.05t) > e^(\u22120.1625t) \u2192 0.05t > \u22120.1625t \u2192 תמיד (A גדל, V קטן). בדקו מתי A(t) = V(t)!" },
    ],
    goldenPrompt: "אתה המורה הפרטי שלי, כיתה יב 5 יחידות.\nרכב 180K, פחת 15%/שנה. חיסכון 180K ריבית רציפה 5%.\nצריך: V(t), A(t), מתי V<50K, מתי A>V, ריבית אפקטיבית." + PF,
    steps: [
      { phase: "א", label: "נוסחאות", coaching: "V=N\u2080\u00B7q\u1D57, A=Pe^(rt)", prompt: "רכב 180K פחת 15%, חיסכון 180K ריבית 5% רציפה. הנחה אותי לכתוב V(t) ו-A(t)." + PF, keywords: ["פחת", "רציפה"], keywordHint: "ציין נוסחאות", contextWords: CW_A },
      { phase: "ב", label: "V<50K", coaching: "ln", prompt: "V(t) = 180000\u00B7(0.85)\u1D57. הנחה אותי למצוא מתי V<50000." + PF, keywords: ["פחת", "ln"], keywordHint: "ציין מתי V<50K", contextWords: CW_A },
      { phase: "ג", label: "A>V", coaching: "השוו e^(0.05t) ל-(0.85)\u1D57", prompt: "הנחה אותי למצוא מתי A(t)>V(t)." + PF, keywords: ["השוואה", "חיתוך"], keywordHint: "ציין A>V", contextWords: CW_A },
      { phase: "ד", label: "ריבית אפקטיבית", coaching: "e^r \u2212 1", prompt: "ריבית רציפה 5%. הנחה אותי לחשב ריבית אפקטיבית שנתית." + PF, keywords: ["אפקטיבית", "e"], keywordHint: "ציין ריבית אפקטיבית", contextWords: CW_A },
    ],
  },
];
...
function GrowthDecayLab() {
  const [P, setP] = useState(10000);
  const [r, setR] = useState(6);
  const [n, setN] = useState(12);

  const rDec = r / 100;
  const annual = P * Math.pow(1 + rDec, 10);
  const compound = P * Math.pow(1 + rDec / n, n * 10);
  const continuous = P * Math.exp(rDec * 10);
  const doubling = Math.log(2) / Math.log(1 + rDec / n) / n;
  const halfLife = Math.log(2) / rDec;

  const cx = 160, cy = 130, sc = 0.4;
  const pts: string[] = [];
  for (let t = 0; t <= 30; t += 0.5) {
    const val = P * Math.pow(1 + rDec / n, n * t);
    const y = cy - (val - P) * sc / 100;
    if (y > 10 && y < 250) pts.push(`${40 + t * 7},${y}`);
  }
  ...
  return (
    <section style={{ border: "1px solid rgba(60,54,42,0.15)", borderRadius: 24, padding: "2.5rem", background: "rgba(255,255,255,0.82)", marginTop: "2rem" }}>
      <h3 style={{ color: "#2D3436", fontSize: 18, fontWeight: 700, marginBottom: 20, textAlign: "center" }}>&#128300; מעבדה — ריבית דריבית</h3>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 12, marginBottom: 20 }}>
        <div><div style={{ display: "flex", justifyContent: "space-between", fontSize: 11, color: "#2D3436", fontWeight: 600, marginBottom: 4 }}><span>קרן P</span><span>{P.toLocaleString()} \u20AA</span></div><input type="range" min={1000} max={100000} step={1000} value={P} onChange={e => setP(Number(e.target.value))} style={{ width: "100%" }} /></div>
        <div><div style={{ display: "flex", justifyContent: "space-between", fontSize: 11, color: "#2D3436", fontWeight: 600, marginBottom: 4 }}><span>ריבית r</span><span>{r}%</span></div><input type="range" min={1} max={20} step={0.5} value={r} onChange={e => setR(Number(e.target.value))} style={{ width: "100%" }} /></div>
        <div><div style={{ display: "flex", justifyContent: "space-between", fontSize: 11, color: "#2D3436", fontWeight: 600, marginBottom: 4 }}><span>צבירות/שנה n</span><span>{n}</span></div><input type="range" min={1} max={365} step={1} value={n} onChange={e => setN(Number(e.target.value))} style={{ width: "100%" }} /></div>
      </div>
      <svg width="100%" viewBox="0 0 300 180" style={{ maxWidth: 420, margin: "0 auto", display: "block" }}>
        <rect x={0} y={0} width={300} height={180} fill="#fafaf5" rx={12} />
        <line x1={40} y1={cy} x2={280} y2={cy} stroke="#94a3b8" strokeWidth={0.7} />
        <line x1={40} y1={170} x2={40} y2={10} stroke="#94a3b8" strokeWidth={0.7} />
        {pts.length > 1 && <polyline points={pts.join(" ")} fill="none" stroke="#ea580c" strokeWidth={2} />}
      </svg>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr 1fr", gap: 8, marginTop: 20 }}>
        <div style={{ borderRadius: 12, border: "1px solid rgba(22,163,74,0.2)", background: "rgba(22,163,74,0.06)", padding: "8px", textAlign: "center" }}><div style={{ fontSize: 9, color: "#6B7280" }}>שנתית (10y)</div><div style={{ fontSize: 13, fontWeight: 700, color: "#16a34a" }}>{annual.toLocaleString(undefined, { maximumFractionDigits: 0 })}</div></div>
        <div style={{ borderRadius: 12, border: "1px solid rgba(234,88,12,0.2)", background: "rgba(234,88,12,0.06)", padding: "8px", textAlign: "center" }}><div style={{ fontSize: 9, color: "#6B7280" }}>n={n} (10y)</div><div style={{ fontSize: 13, fontWeight: 700, color: "#ea580c" }}>{compound.toLocaleString(undefined, { maximumFractionDigits: 0 })}</div></div>
        <div style={{ borderRadius: 12, border: "1px solid rgba(220,38,38,0.2)", background: "rgba(220,38,38,0.06)", padding: "8px", textAlign: "center" }}><div style={{ fontSize: 9, color: "#6B7280" }}>רציפה (10y)</div><div style={{ fontSize: 13, fontWeight: 700, color: "#dc2626" }}>{continuous.toLocaleString(undefined, { maximumFractionDigits: 0 })}</div></div>
        <div style={{ borderRadius: 12, border: "1px solid rgba(124,58,237,0.2)", background: "rgba(124,58,237,0.06)", padding: "8px", textAlign: "center" }}><div style={{ fontSize: 9, color: "#6B7280" }}>זמן הכפלה</div><div style={{ fontSize: 13, fontWeight: 700, color: "#7c3aed" }}>{doubling.toFixed(1)} שנים</div></div>
      </div>
      <p style={{ textAlign: "center", color: "#6B7280", fontSize: 12, marginTop: 14 }}>שנו קרן, ריבית ותדירות צבירה — ראו את ההשפעה על הגדילה</p>
    </section>
  );
}
...
export default function HalfLifeInterestPage() {
    ...
    <main dir="rtl" style={{ minHeight: "100vh", background: "#F3EFE0", backgroundImage: "radial-gradient(rgba(60,54,42,0.07) 1px, transparent 1px)", backgroundSize: "24px 24px" }}>
      ...
      <div style={{ borderBottom: "1px solid rgba(60,54,42,0.15)", background: "#F3EFE0" }}>
        <div style={{ maxWidth: "56rem", margin: "0 auto", padding: "0.9rem 1.5rem", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          <h1 style={{ fontSize: 22, fontWeight: 700, color: "#2D3436", margin: 0 }}>חצי חיים וריבית</h1>
          <Link href="/5u/topic/grade12/growth-decay" style={{ display: "flex", alignItems: "center", gap: 6, padding: "8px 14px", background: "rgba(148,163,184,0.1)", border: "1px solid rgba(60,54,42,0.15)", borderRadius: 10, fontSize: 14, fontWeight: 600, color: "#2D3436", textDecoration: "none" }}>&#8592; חזרה</Link>
        </div>
      </div>
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }} style={{ maxWidth: "56rem", margin: "0 auto", padding: "2rem 1rem 4rem" }}>
        <SubtopicProgress subtopicId="5u/grade12/growth-decay/half-life-interest" />
        ...
        <GrowthDecayLab />
        <div style={{ marginTop: "2rem" }}><MarkComplete subtopicId="5u/grade12/growth-decay/half-life-interest" level={lvl} /></div>
      </motion.div>
    </main>
  );
}
