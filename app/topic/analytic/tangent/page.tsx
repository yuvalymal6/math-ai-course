...
const EXERCISES: ExerciseDef[] = [
  {
    ...
    steps: [
      { phase: "א", label: "שלב א׳ — שיפוע הרדיוס לנקודה (3,4)", prompt: "\n\nמרכז המעגל (0,0), נקודה על המעגל (3,4). הנחה אותי לחשב את שיפוע הרדיוס מהמרכז לנקודה. אל תפתור בשבילי — שאל אותי מה הנוסחה." },
      { phase: "ב", label: "שלב ב׳ — תנאי ניצבות למציאת שיפוע המשיק", prompt: "\n\nמצאתי את שיפוע הרדיוס. הנחה אותי להשתמש בתנאי הניצבות m₁×m₂=−1 כדי למצוא את שיפוע המשיק. אל תחשב בשבילי." },
      { phase: "ג", label: "שלב ג׳ — אמת: מכפלת השיפועים = −1", prompt: "\n\nמצאתי את שני השיפועים. הנחה אותי לבדוק אם מכפלתם שווה ל-−1. אם לא — איפה טעיתי?" },
      { phase: "ד", label: "שלב ד׳ — כתוב משוואת הישר המשיק", prompt: "\n\nיש לי את שיפוע המשיק ואת נקודת המשיקות (3,4). הנחה אותי לכתוב משוואת ישר y−y₁=m(x−x₁) ולפשט. אל תפתור." },
    ],
  },
  {
    id: "medium",
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                {ex.pitfalls.map((p, i) => (
                  <div key={i} style={{ borderRadius: 12, border: "1px solid rgba(220,38,38,0.2)", background: "rgba(254,226,226,0.25)", padding: "10px 14px" }}>
                    <div style={{ color: "#b91c1c", fontWeight: 700, fontSize: 13, marginBottom: 3 }}>{p.title}</div>
