...
// Subject words for MasterPromptGate (root-function-specific)
const ROOT_SUBJECT_WORDS = [
  "שורש", "תחום", "פונקציית שורש", "מתחת לשורש", "נקודת קצה", "אי-שלילי", "כלל השרשרת", "√",
];

// ─── FunctionGraph ────────────────────────────────────────────────────────────
// Keeps isFinite + isNaN checks — critical for root functions (negative inner → NaN).
...
// ─── RootLab ──────────────────────────────────────────────────────────────────
// Domain explorer: slider for k in √(x−k).

function RootLab({ levelId }: { levelId: "basic" | "medium" | "advanced" }) {
  const [k, setK] = useState(1.0);
  const st = STATION[levelId];
  const W = 300, H = 200;
  const xMin = -3, xMax = 8, yMin = -0.4, yMax = 3.5;
  const toSx = (x: number) => ((x - xMin) / (xMax - xMin)) * W;
  const toSy = (y: number) => H - ((y - yMin) / (yMax - yMin)) * H;
  ...
  const ox = toSx(0), oy = toSy(0);
  const kx = toSx(k);
  const forbiddenWidth = Math.max(0, Math.min(kx, W));
  const startSx = kx, startSy = toSy(0);
  const dotVisible = startSx >= 0 && startSx <= W;
  const fmt = (v: number) => v % 1 === 0 ? v.toFixed(0) : v.toFixed(1);
  const domainLabel = `[${fmt(k)}, ∞)`;
  const startLabel = `(${fmt(k)}, 0)`;
  ...
  return (
    <section style={{ border: `1px solid ${st.glowBorder}`, borderRadius: 24, padding: "2.5rem", background: "rgba(255,255,255,0.82)", backdropFilter: "blur(8px)", marginLeft: "auto", marginRight: "auto", boxShadow: "0 10px 15px -3px rgba(60,54,42,0.1)" }}>
      ...
      <p style={{ color: "#6B7280", fontSize: 14, textAlign: "center", marginBottom: "2rem" }}>
        f(x) = √(x − <span style={{ color: st.accentColor, fontFamily: "monospace", fontWeight: 700 }}>{fmt(k)}</span>)
        {" "}— שנה את k וראה כיצד התחום זז
      </p>
      ...
      <div style={{ background: "rgba(255,255,255,0.75)", borderRadius: 16, border: "1px solid rgba(255,255,255,0.4)", padding: "1.25rem", boxShadow: "0 4px 16px rgba(60,54,42,0.12)", marginBottom: "2rem" }}>
        <div style={{ display: "flex", justifyContent: "space-between", fontSize: 12, color: "#6B7280", marginBottom: 4 }}>
          <span>נקודת התחלה <span style={{ color: st.accentColor, fontFamily: "monospace", fontWeight: 600 }}>k</span></span>
          <span style={{ color: st.accentColor, fontFamily: "monospace", fontWeight: 700 }}>{fmt(k)}</span>
        </div>
        <input type="range" min="-3" max="6" step="0.5" value={k} onChange={(e) => setK(parseFloat(e.target.value))} style={{ width: "100%", accentColor: st.accentColor }} />
        <div style={{ display: "flex", justifyContent: "space-between", fontSize: 10, color: "#9CA3AF", marginTop: 2 }}>
          <span>−3</span><span>0</span><span>+6</span>
        </div>
      </div>
      ...
      <div style={{ borderRadius: 16, border: `1px solid ${st.glowBorder}`, background: "rgba(255,255,255,0.75)", padding: "1.5rem", display: "flex", alignItems: "center", justifyContent: "center", marginBottom: "2rem", boxShadow: st.glowShadow }}>
        <svg viewBox={`0 0 ${W} ${H}`} style={{ width: "100%", maxWidth: 320 }} aria-hidden>
          {/* Forbidden zone */}
          {forbiddenWidth > 0 && (
            <rect x={0} y={0} width={forbiddenWidth} height={H} fill="#94a3b8" opacity={0.12} />
          )}
          {/* Grid */}
          {[...Array(5)].map((_, i) => {
            const gy = (H / 4) * i;
          ...
          {oy >= 0 && oy <= H && <line x1={0} y1={oy} x2={W} y2={oy} stroke="#94a3b8" strokeWidth={1.2} />}
          {ox >= 0 && ox <= W && <line x1={ox} y1={0} x2={ox} y2={H} stroke="#94a3b8" strokeWidth={1.2} />}
          {/* Domain boundary dashed line */}
          {kx >= 0 && kx <= W && (
            <line x1={kx} y1={0} x2={kx} y2={H} stroke="#64748b" strokeWidth={1} strokeDasharray="3 3" opacity={0.5} />
          )}
          {/* Curve */}
          <path d={segs.join(" ")} fill="none" stroke={st.accentColor} strokeWidth={2.5} strokeLinecap="round" strokeLinejoin="round" />
          {/* Starting point dot */}
          {dotVisible && (
            <circle cx={startSx} cy={startSy} r={5} fill={st.accentColor} />
          )}
        </svg>
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 12, textAlign: "center" }}>
        <div style={{ borderRadius: 16, background: "rgba(255,255,255,0.75)", border: `1px solid rgba(${st.glowRgb},0.4)`, padding: 14, boxShadow: "0 4px 16px rgba(60,54,42,0.06)" }}>
          ...
          <div style={{ color: st.accentColor, fontWeight: 700, fontSize: 13 }}>{domainLabel}</div>
        </div>
        <div style={{ borderRadius: 16, background: "rgba(255,255,255,0.75)", border: `1px solid rgba(${st.glowRgb},0.4)`, padding: 14, boxShadow: "0 4px 16px rgba(60,54,42,0.06)" }}>
          <div style={{ color: "#6B7280", fontSize: 10, marginBottom: 6 }}>נקודת התחלה</div>
          <div style={{ color: "#16a34a", fontWeight: 700, fontSize: 13 }}>{startLabel}</div>
        </div>
        <div style={{ borderRadius: 16, background: "rgba(255,255,255,0.75)", border: `1px solid rgba(${st.glowRgb},0.4)`, padding: 14, boxShadow: "0 4px 16px rgba(60,54,42,0.06)" }}>
          <div style={{ color: "#6B7280", fontSize: 10, marginBottom: 6 }}>אזור אסור</div>
          <div style={{ color: "#6b7280", fontWeight: 700, fontSize: 13 }}>x &lt; {fmt(k)}</div>
        </div>
      </div>
      <p style={{ color: "#6B7280", fontSize: 12, textAlign: "center", marginTop: "1rem" }}>
        הנקודה הכחולה היא נקודת ההתחלה — גם נקודת מינימום מוחלטת של הפונקציה
      </p>
    </section>
  );
}
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      <MasterPromptGate
        ...
        subjectWords={ROOT_SUBJECT_WORDS}
        subjectHint="פונקציית שורש / תחום / נקודת קצה / כלל השרשרת"
        ...
        <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.45, ease: "easeOut" }}>
          ...
          {allPassed && (
            ...
            >
              ...
              <div style={{ color: "#166534", fontSize: 13, marginTop: 4 }}>מציאת פרמטרים מנקודות נתונות — מאסטר בפונקציות שורש!</div>
            </motion.div>
          )}
...
const exercises: ExerciseDef[] = [
  // ── מתחיל: תחום ונקודת התחלה ──────────────────────────────────────────────
  {
    id: "basic",
    problem: "נתונה הפונקציה:\nf(x) = √(x − 4)\n\nמצא את התחום ואת נקודת ההתחלה.",
    diagram: (
      <FunctionGraph
        fn={(x) => Math.sqrt(x - 4)}
        xMin={-1} xMax={13} yMin={-0.3} yMax={3.5}
        color="#3b82f6"
      />
    ),
    pitfalls: [
      { title: "⚠️ שכחת שאסור להציב שורש שלילי", text: "הביטוי מתחת לשורש חייב להיות ≥ 0 (לא רק > 0). הנקודה x=4 כן שייכת לתחום." },
      { title: "⚠️ נקודת התחלה היא גם נקודת קיצון", text: "הפונקציה מתחילה ב-(4,0) ועולה משם. f'(x) → ∞ ב-x=4, אבל הנקודה עצמה קיימת ומהווה מינימום מוחלט." },
    ],
    ...
    steps: [
      {
        phase: "שלב א׳",
        label: "תנאי הגדרה — מה חייב להיות אי-שלילי?",
        prompt: "\n\nf(x) = √(x-4). מה התנאי על x כדי שהביטוי מתחת לשורש יהיה מוגדר?",
      },
      {
        phase: "שלב ב׳",
        label: "כתוב את התחום בסימון קבוצתי",
        prompt: "\n\nf(x) = √(x-4). כתוב את תחום ההגדרה בסימון קבוצתי.",
      },
      {
        phase: "שלב ג׳",
        label: "חשב את נקודת ההתחלה",
        prompt: "\n\nf(x) = √(x-4). חשב f(4) ומצא את נקודת ההתחלה של הפונקציה.",
      },
      {
        phase: "שלב ד׳",
        label: "מדוע (4,0) היא גם נקודת מינימום?",
        prompt: "\n\nf(x) = √(x-4). מדוע הנקודה (4,0) נחשבת נקודת מינימום של הפונקציה?",
      },
    ],
  },

  // ── בינוני: תחום מורכב וקטעי עלייה/ירידה ─────────────────────────────────
  {
    id: "medium",
    problem: "נתונה הפונקציה:\nf(x) = √(x² − 9)\n\nמצא את התחום וקבע קטעי עלייה וירידה.",
    diagram: (
      <FunctionGraph
        fn={(x) => Math.sqrt(x * x - 9)}
        xMin={-7} xMax={7} yMin={-0.3} yMax={4.5}
        color="#a78bfa"
      />
    ),
    pitfalls: [
      { title: "⚠️ שוכחים את הענף השמאלי", text: "x²-9 ≥ 0 מתקיים גם עבור x ≤ -3, לא רק x ≥ 3. יש שני קטעי תחום!" },
      { title: "⚠️ כלל השרשרת בגזירה", text: "f'(x) = x / √(x²-9). הנגזרת הפנימית של x² היא 2x, ויחד עם הכלל החיצוני מקבלים x/√(x²-9)." },
    ],
    ...
    steps: [
      {
        phase: "שלב א׳",
        label: "פתור את האי-שוויון x²-9 ≥ 0",
        contextWords: ["x²-9", "תחום", "אי-שוויון", "x≤-3", "x≥3"],
      },
      {
        phase: "שלב ב׳",
        label: "כתוב את התחום המלא בסימון קבוצתי",
        contextWords: ["תחום", "(-∞,-3]", "[3,∞)", "סימון קבוצתי", "∪"],
      },
      {
        phase: "שלב ג׳",
        label: "גזור בכלל השרשרת",
        contextWords: ["כלל השרשרת", "נגזרת", "f'(x)", "x/√", "2x"],
      },
      {
        phase: "שלב ד׳",
        label: "קבע קטעי עלייה/ירידה",
        contextWords: ["f'(x)", "עולה", "יורדת", "סימן", "x>3"],
      },
    ],
  },

  // ── מתקדם: מציאת פרמטרים מנקודות נתונות ──────────────────────────────────
  {
    id: "advanced",
    problem: "f(x) = √(ax + b)\n\nידוע:\n• הפונקציה מתחילה בנקודה (2, 0)\n• הפונקציה עוברת דרך (6, 4)\n\nמצא את a ו-b.",
    diagram: (
      <FunctionGraph
        fn={(x) => Math.sqrt(4 * x - 8)}
        xMin={-0.5} xMax={8} yMin={-0.3} yMax={4.5}
        color="#f43f5e"
      />
    ),
    pitfalls: [
      { title: "⚠️ מציבים f(2)=0 בלבד", text: "נקודת התחלה (2,0) אומרת שהביטוי מתחת לשורש שווה 0 ב-x=2: 2a+b=0. צריך עוד משוואה מהנקודה השנייה." },
      { title: "⚠️ שוכחים לרבע", text: "√(6a+b) = 4 → יש לרבע את שני הצדדים: 6a+b = 16. לא 6a+b = 4!" },
    ],
    goldenPrompt: "\n\n",
    steps: [
      {
        phase: "שלב א׳",
        label: "השתמש בנקודת ההתחלה (2,0)",
        contextWords: ["נקודת התחלה", "2a+b=0", "הצב", "x=2", "אפס"],
      },
      {
        phase: "שלב ב׳",
        label: "הצב את הנקודה (6,4) ורבע",
        contextWords: ["6a+b=16", "רבע", "הצב", "x=6", "√(6a+b)=4"],
      },
      {
        phase: "שלב ג׳",
        label: "פתור מערכת משוואות — מצא a ו-b",
        contextWords: ["מערכת", "a=4", "b=-8", "פתרון", "חסרים"],
      },
      {
        phase: "שלב ד׳",
        label: "כתוב f(x) ואמת את שתי הנקודות",
        contextWords: ["f(2)=0", "f(6)=4", "אמת", "√(4x-8)", "✓"],
      },
    ],
  },
];
  ...
  return (
    <section style={{ border: `1px solid ${s.glowBorder}`, borderRadius: 24, padding: "2.5rem", background: "rgba(255,255,255,0.82)", backdropFilter: "blur(8px)", marginLeft: "auto", marginRight: "auto", boxShadow: "0 10px 15px -3px rgba(60,54,42,0.1)" }}>
      ...
      <div style={{ borderRadius: 16, border: `1px solid ${s.glowBorder}`, background: "rgba(255,255,255,0.75)", padding: "1.25rem 1.5rem", marginBottom: "2rem", boxShadow: s.glowShadow }}>
        <div style={{ color: "#6B7280", fontSize: 10, textTransform: "uppercase", letterSpacing: "0.12em", fontWeight: 700, marginBottom: 12, textAlign: "center" }}>כלי פונקציות שורש</div>
        <div style={{ display: "flex", gap: "2rem", flexWrap: "wrap", justifyContent: "center" }}>
          <div style={{ display: "flex", flexDirection: "column", gap: 5, fontSize: 13, color: "#2D3436", lineHeight: 1.7 }}>
            <span><span style={{ color: s.accentColor, fontFamily: "monospace", fontWeight: 700 }}>√u ≥ 0</span> → ביטוי תחת שורש חייב להיות אי-שלילי</span>
            <span><span style={{ color: s.accentColor, fontFamily: "monospace", fontWeight: 700 }}>D(f)</span> → פותרים u ≥ 0 כדי לקבל תחום</span>
            <span><span style={{ color: s.accentColor, fontFamily: "monospace", fontWeight: 700 }}>נקודת קצה</span> → גם נקודת מינימום מוחלטת</span>
          </div>
          <div style={{ width: 1, background: "rgba(60,54,42,0.1)", alignSelf: "stretch" }} />
          <div style={{ display: "flex", flexDirection: "column", gap: 5, fontSize: 13, color: "#2D3436", lineHeight: 1.7 }}>
            <span><span style={{ color: "#a78bfa", fontFamily: "monospace", fontWeight: 700 }}>(√u)&apos;</span> = u&apos; / (2√u) — כלל השרשרת</span>
      ...
      <div style={{ borderBottom: "1px solid rgba(60,54,42,0.15)", background: "#F3EFE0" }}>
        <div style={{ maxWidth: "56rem", margin: "0 auto", padding: "0.9rem 1.5rem", display: "flex", alignItems: "center", justifyContent: "space-between", gap: "1rem" }}>
          <div>
            ...
            <p style={{ fontSize: 13, color: "#6B7280", margin: "2px 0 0" }}>תחום, נקודות קצה ועלייה/ירידה — ואיך לנסח ל-AI את השאלות הנכונות</p>
          </div>
          <Link
            href="/topic/calculus"
