...
import { Brain, ChevronRight, Box, Triangle } from "lucide-react";
...
const SUBTOPICS = [
  {
    id: "box",
    href: "/topic/grade12/space-geometry/box",
    icon: Box,
    title: "התיבה והמנסרה",
    subtitle: "נפח • שטח פנים • אלכסון מרחבי • זוויות",
    description: "חישוב נפח ושטח פנים של תיבה ומנסרה, מציאת אורך האלכסון המרחבי, וקביעת הזווית שבין האלכסון למישורי הבסיס.",
    chips: ["V = a·b·c", "d = √(a²+b²+c²)", "זווית אלכסון"],
    ready: true,
    glowAnim: true,
  ...
  {
    id: "pyramid",
    href: "/topic/grade12/space-geometry/pyramid",
    icon: Triangle,
    title: "הפירמידה הישרה",
    subtitle: "נפח • גובה • אפותמה • זוויות",
    description: "חישוב גובה הפירמידה, נפחה ושטח פניה הצדדיים, מציאת האפותמה (גובה הפאה המשולשת), וניתוח זוויות בין הפאות למישור הבסיס.",
    chips: ["V = ⅓·B·h", "אפותמה", "זווית פאה"],
    ready: true,
    glowAnim: false,
...
export default function SpaceGeometryHub() {
  return (
    <div className="min-h-screen bg-[#0a0f1e] text-white" dir="rtl">
      <style>{`
        @keyframes fadeSlideIn { from { opacity:0; transform:translateY(10px); } to { opacity:1; transform:translateY(0); } }
        @keyframes pulseGlowEmerald { 0%,100% { box-shadow: 0 0 0 0 rgba(52,211,153,0); } 50% { box-shadow: 0 0 20px 5px rgba(52,211,153,0.13); } }
      ...
      <main className="max-w-2xl mx-auto px-4 py-14 space-y-10">
        ...
        <div className="text-center space-y-3 animate-[fadeSlideIn_0.4s_ease_both]">
          <p className="text-emerald-400 text-xs font-semibold uppercase tracking-widest">כיתה י״ב • 4 יח&quot;ל</p>
          <h1 className="text-4xl font-extrabold">גאומטריה במרחב</h1>
          <p className="text-slate-400 text-sm">תיבה • פירמידה • זוויות ואורכים במרחב</p>
        </div>
          ...
          {SUBTOPICS.map((sub) => {
            ...
            return (
              <Link
                ...
                style={sub.glowAnim ? { animation: "pulseGlowEmerald 3s ease-in-out infinite" } : undefined}
              >
                <div className="flex items-start gap-5">
