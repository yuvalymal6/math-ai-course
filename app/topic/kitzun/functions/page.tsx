...
const FORMULAS = [
  { label: "שטח מלבן",          formula: "A(x) = 2x·f(x)" },
  { label: "מרחק²",             formula: "D²=(x₁−x₂)²+(y₁−y₂)²" },
  { label: "קיצון",             formula: "A′(x) = 0" },
  { label: "נגזרת f(x)=−x²+9", formula: "f′(x) = −2x" },
];
        ...
        <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.45, ease: "easeOut" }}>
          ...
          {allPassed && (
            <div style={{ borderRadius: 16, background: "rgba(220,252,231,1)", border: "2px solid #16a34a", padding: "1rem 1.5rem", marginTop: 16, textAlign: "center" }}>
              ...
              <p style={{ color: "#14532d", fontWeight: 700, fontSize: 15, margin: "4px 0 0" }}>מלבן מקסימלי בין פרבולות — מאסטר בחדו&quot;א!</p>
            </div>
          )}
        </motion.div>
      )}
...
const exercises: ExerciseDef[] = [
  {
    ...
    steps: [
      ...
      { phase: "שלב 4", label: "חשב שטח מקסימלי", prompt: "\n\nx = √3. הנחה אותי לחשב A(√3), רוחב = 2√3 וגובה = 9 − 3. מה השטח המקסימלי?" },
    ],
  },
  {
    id: "medium",
